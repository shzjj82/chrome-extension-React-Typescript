...
import { isFilesPath, isStudyPath, PATHS } from '../../lib/routes';
...
const TITLE_BY_PATH: Record<string, string> = {
  ...
  [PATHS.organize]: '开始整理',
  [PATHS.study]: '学习',
  [PATHS.studyTab('organize')]: '整理',
  [PATHS.studyTab('exam')]: '考试',
  [PATHS.studyTab('review')]: '复习',
};
...
const AppPageHeader = ({ defaultOnBack }: AppPageHeaderProps) => {
  ...
  const fallbackTitle = isFilesPath(location.pathname)
    ? ''
    : isStudyPath(location.pathname)
      ? (TITLE_BY_PATH[location.pathname] ?? '学习')
